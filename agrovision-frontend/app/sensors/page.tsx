...
export default function SensorsPage() {
  ...
  const avg = (arr: Sensor[]) =>
    arr.length
      ? (arr.slice(0, 5).reduce((a, b) => a + b.value, 0) / Math.min(arr.length, 5)).toFixed(1)
      : '—';
      ...
      <main style={{ marginLeft: '240px', flex: 1, padding: '28px 32px' }}>
        ...
        <div style={{ marginBottom: '24px' }}>
          ...
          <p style={{ margin: 0, fontSize: '13px', color: '#64748b' }}>
            Lecturas en tiempo real · {sensors.length} total registradas
          </p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '20px' }}>
          <div style={{
            ...
            <div>
              <p style={{ margin: 0, fontSize: '11px', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                Temp. Promedio (últimas 5)
              </p>
              <p style={{ margin: '4px 0 0', fontSize: '28px', fontWeight: 700, color: '#f59e0b' }}>
                {avg(temperature)}°C
              </p>
              <p style={{ margin: '2px 0 0', fontSize: '12px', color: '#64748b' }}>
                {temperature.length} lecturas totales
              </p>
            </div>
          </div>
          ...
          <div style={{
            ...
            <div>
              <p style={{ margin: 0, fontSize: '11px', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                Humedad Promedio (últimas 5)
              </p>
              <p style={{ margin: '4px 0 0', fontSize: '28px', fontWeight: 700, color: '#3b82f6' }}>
                {avg(humidity)}%
              </p>
              <p style={{ margin: '2px 0 0', fontSize: '12px', color: '#64748b' }}>
                {humidity.length} lecturas totales
              </p>
            </div>
          </div>
        </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
            <div>
              ...
              <p style={{ margin: '3px 0 0', fontSize: '12px', color: '#64748b' }}>
                Últimas {Math.min(temperature.length, 30)} lecturas
              </p>
            </div>
            <div style={{ display: 'flex', gap: '16px', fontSize: '12px' }}>
              ...
              <span style={{ display: 'flex', alignItems: 'center', gap: '6px', color: '#3b82f6' }}>
                <span style={{ width: '24px', height: '2px', background: '#3b82f6', display: 'inline-block', borderRadius: '1px' }} />
                Humedad
              </span>
            </div>
          </div>
          <SensorChart sensors={sensors} limit={30} height={300} />
        </div>
        ...
        <div style={{
          ...
          <h2 style={{ margin: '0 0 16px', fontSize: '14px', fontWeight: 600, color: '#f1f5f9' }}>
          ...
          {fetching ? (
            ...
            <p style={{ color: '#64748b', textAlign: 'center', padding: '48px', fontSize: '13px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: '420px', overflowY: 'auto' }}>
              {sensors.map((s) => {
                ...
                const bg = isTemp ? 'rgba(245,158,11,0.06)' : 'rgba(59,130,246,0.06)';
                const border = isTemp ? 'rgba(245,158,11,0.12)' : 'rgba(59,130,246,0.12)';
                return (
                  <div key={s._id} style={{
                    ...
                    <div style={{
                      ...
                      background: isTemp ? 'rgba(245,158,11,0.15)' : 'rgba(59,130,246,0.15)',
                      borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center',
                      flexShrink: 0,
                    }}>
                      {isTemp
                        ? <ThermometerIcon size={14} color={color} />
                        : <DropletIcon size={14} color={color} />}
                    </div>
                    <div style={{ flex: 1 }}>
